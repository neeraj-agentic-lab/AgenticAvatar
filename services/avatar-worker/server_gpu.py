"""
GPU avatar worker — offline batch mode using Ditto's StreamSDK.

Flow per turn:
  1. Receives PCM audio chunks via gRPC stream
  2. On IDLE signal: runs Ditto inference (offline batch)
  3. Reads output MP4 frame by frame and yields as gRPC RenderOutput
"""
...
def _load_ditto():
    ...
    log.info("Ditto ready. Portrait: %s", SOURCE_IMAGE)
...
class AvatarRendererServicer(avatar_pb2_grpc.AvatarRendererServicer):
    ...
    async def Stream(self, request_iterator, context):
        ...
        async for msg in request_iterator:
            ...
            if msg.HasField("control"):
                ...
                if len(pcm_buf) % (SAMPLE_RATE * 2) == 0:
    ...
    async def _run_inference(self, pcm: bytes, session_id: str, turn_id: str, generation: int):
        if self._sdk is None or self._ditto_run is None:
            log.warning("Ditto not ready, skipping inference")
            return
        ...
        try:
            ...
            with self._lock:
                ...
                await loop.run_in_executor(
                    None,
                    lambda: self._ditto_run(self._sdk, wav_path, SOURCE_IMAGE, out_video),
                )

            # Delete WAV after ditto_run (which includes internal ffmpeg) completes
            ...
            log.info("Inference done %.2fs — output exists=%s size=%d bytes", elapsed, exists, size)
            ...
            log.info("Yielded %d frames", frame_count)
